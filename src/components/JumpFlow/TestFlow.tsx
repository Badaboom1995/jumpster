...
import { detectHips, drawPoses, drawVideoFrame, startCamera} from "@/components/JumpFlow/utils";
...
const VideoCanvas = () => {
    ...
    return (
        <div className={`w-full h-full fixed top-0 left-0 z-50 bg-slate-800 ${hipsDetected && Math.abs(moveVectorY.currentVector) < 10 && isRunning && "animate-flash"}`}>
            ...
            <canvas ref={canvasRef} className="w-full h-full -scale-x-100" />
             <div className="absolute top-[24px] left-1/2 -translate-x-1/2 text-white p-4">
                 ...
                 {/*<p>{moveVectorY.standStill && 'stand still'}</p>*/}
                 {/*<p>{Math.ceil(moveVectorY.prevValue)}</p>*/}
                 {/*<p>{isRunning && 'Running'}</p>*/}
                 {/*<p>Seconds: {seconds}</p>*/}
                 {/*<p>{jumpStarted && 'Jump started'}</p>*/}
                 {/*<p className='text-[64px]'>{`${jumpsCounter}`}</p>*/}
                 {/*{!flowStarted || !hipsDetected && <p>Встаньте так чтобы было видно бедра</p>}*/}
                 {/*{!jumpStarted && hipsDetected && Math.abs(moveVectorY.currentVector) > 10 && <p>Займите удобную позицию и не двигайтесь</p>}*/}
                 {/*{!jumpStarted && hipsDetected && Math.abs(moveVectorY.currentVector) < 10 && <div><p>Стойте спокойно</p><p>Сессия начнется через {seconds}</p></div>}*/}
                 {/*{jumpStarted && <p>Прыгайте!</p>}*/}
                 {/*<div>{!isRunning && !seconds && <p>Прыгай!</p>}</div>*/}
             </div>

            {/*{buffer.length > 0 && <div className="absolute bottom-0 left-0 text-white p-4">Buffer Length: {buffer.length}</div>}*/}
        </div>
    );
};
