...
export const setMoveVector = (keypoints: any, setVector: any) => {
  ...
  if (leftHip.score > 0.5 && rightHip.score > 0.5) {
    ...
    setVector((prev: any) => {
      if (prev.prevValue === 0)
        return { prevValue: avgHipY, currentVector: 0, standStill: false };
      ...
      if (Math.abs(prev.currentVector - nextValue) < 5) {
        return {
          ...
          currentVector: prev.currentVector + gravity,
          standStill: Math.abs(prev.currentVector + gravity) < 10,
        };
      }
      return {
        prevValue: avgHipY,
        currentVector: nextValue,
        standStill: Math.abs(nextValue) < 10,
      };
    });
  } else {
